...
const CalendarRoot = styled(Box)(({ theme, ownerState }) => {
...
const Calendar = ({ header, ...rest }) => {
  ...
  return (
    <Card sx={{ height: '100%' }}>
      <CustomBox pt={2} px={2} lineHeight={1}>
        {header.title ? (
        ...
        {header.date ? (
          <Typography component="p" variant="button" color="text" fontWeight="medium">
            {header.date}
          </Typography>
        ) : null}
      </CustomBox>
      <CalendarRoot p={2} ownerState={{ darkMode: false }}>
        <FullCalendar
          {...rest}
